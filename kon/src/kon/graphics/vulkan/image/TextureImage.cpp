...
namespace kon
{
	TextureImage::TextureImage(Device *device, CommandPool *commandPool, u8 *image, const TextureData &data, bool mipmap)
		: m_device(device), m_commandPool(commandPool), m_width(data.width), m_height(data.height)
	{
		KN_INSTRUMENT_FUNCTION()

		m_mipLevels = static_cast<uint32_t>(std::floor(std::log2(std::max(data.width, data.height)))) + 1;

		Buffer *stagingBuffer = new Buffer(device, commandPool, data.size,
				VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

		stagingBuffer->BindData(image, data.size);

		m_image = new Image(device, data.width, data.height, m_mipLevels, 
			VK_SAMPLE_COUNT_1_BIT,
         	VK_FORMAT_R8G8B8A8_SRGB,
          	VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
          	VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

		m_image->TransitionImageLayout(commandPool,
    ...
    void TextureImage::GenerateMipmaps(VkFormat imageFormat)
    {
        KN_INSTRUMENT_FUNCTION()
		...
		{
            KN_WARN("texture image format does not support linear blitting!");
        }
